...
class IVT_MC(IProbNorm):

    def fit(self, x : np.array, y : np.array, x_val : np.array, y_val : np.array, epochs, epochs_pi=0, M=1):
        """
        Train the interval-based model using the Monte Carlo approach.
        ...
        epochs : int
            Number of attention-training epochs (complete passes through training data).
        epochs_pi : int
            Number of Pi-training epochs (complete passes through training data).
        M : int
            Specifies the number of probability generations (matrix S).
        """
        ...
        self.best_val_loss_pi = sys.float_info.max
        self.best_epoch = 0
        self.best_epoch_pi = 0
        ...
        self.S_norm = self._unwrap_norm_matrix(self._generate_norm_matrix(M=self.M), omega_unpack=False)
        self.S_norm_best = self.S_norm
        ...
        for epoch in range(epochs):
            ...
            self.S_norm = self._generate_norm_matrix(M=self.M)
            S_unwrapped = self._unwrap_norm_matrix(self.S_norm, omega_unpack=False)
            ...
            for query_num, query_idx in enumerate(query_indices):
                ...
                A = A.unsqueeze(dim=2).repeat(1, 1, self.M)
                probs = torch.einsum('kbm,btm->ktm', A, S)

                loss_train_batch = self.compute_L_main(P=probs, k=k, is_train=True, indicies=query_idx) + self.compute_L2()
                ...
                self.brier_train[-1] += score_brier_batch
                self.log_losses_train[-1] += log_losses_batch
                batches += 1
            ...
            if x_val is not None:
                with torch.no_grad():
                    ...
                    for query_num, query_idx in enumerate(batch_indices):
                        ...
                        A = A.unsqueeze(dim=2).repeat(1, 1, self.M)
                        probs = torch.einsum('kbm,btm->ktm', A, S_unwrapped)

                        loss = float(self.compute_L_main(P=probs, k=k, is_train=False, indicies=query_idx) + self.compute_L2())
                        score_brier = float(self.compute_BrierScore(P=probs, is_train=False, indicies=query_idx).item())
                    ...
                    if self.losses_val[-1] < self.best_val_loss:
                        self.S_norm_best = self.S_norm.clone()
        ...
        self.S_norm = self.S_norm_best.clone()
        ...
        scheduler = None
        if self._is_learnable():
            optimizer = self._create_optimizer(learn_sigma=True, learn_mu=False, learn_attention=False)
            scheduler = self._create_scheduler(optimizer) if self.lr_scheduler else None
        ...
        for epoch in range(epochs_pi):
            ...
            if x_val is not None:
                with torch.no_grad():
                    ...
                    log_str += f"val={self.losses_val[-1]} "
                    log_str += f"val_br={self.brier_val[-1]} "
                    if self.losses_val[-1] < self.best_val_loss_pi:
                        self._S_mu_params_best = self._S_mu_params.clone()
                        self._S_omega_params_best = self._S_omega_params.clone()
                        self.best_val_loss_pi = self.losses_val[-1]
                        self.best_epoch_pi = epoch + epochs
